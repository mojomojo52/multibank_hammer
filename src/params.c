...
{
	fprintf(stderr,
		"[ HELP ] - Usage ./%s [-h] [-r rounds] [-a aggr] [-o o_file] [-v] [-m] [-d] [-c] [--mem mem_size] [--[huge/HUGE] f_name] [--conf f_name] [--align val] [--off val] [--no-overwrite] [--fuzzing]\n",
		bin_name);
	...
	//fprintf(stderr, "\t-f\t\t\t= Start fuzzing (--aggr will be ignored)\n");
	//fprintf(stderr, "\t-g\t\t\t= Run group hammer\n");
	fprintf(stderr, "\t-t --threshold\t\t= Align the hammering to refresh ops,\n\t\t\t\t looking at the memory latency in CPU cycles.\t(default: 0)\n");
	//fprintf(stderr, "\t-d\t\t\t= Distance between sets\n");
}
...
int process_argv(int argc, char *argv[], ProfileParams *p)
{
	...
	const struct option long_options[] = {
		...
		//{.name = "find_vic",.has_arg = no_argument,.flag = &p->find_vic, .val = 1},
		//{.name = "dist",.has_arg = required_argument,.flag = NULL,.val ='d'},
		//{.name = "vics",.has_arg = required_argument,.flag = NULL,.val ='c'},
	...
	while (1) {
		...
		int option_index = 0;
		//int arg = getopt_long(argc, argv, "o:r:hvV:T:a:fgc:d:t:F",
		//		      long_options, &option_index);
		int arg = getopt_long(argc, argv, "o:vt:",
				      long_options, &option_index);
		if (arg == -1)
			break;
